...
class CustomGradScaler(GradScaler):
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    # Custom
    # =======================
    def _maybe_opt_step_first(self, optimizer, optimizer_state, *args, **kwargs):
        retval = None
        if not sum(v.item() for v in optimizer_state["found_inf_per_device"].values()):
            retval = optimizer.first_step(*args, **kwargs)
        return retval

    def first_step(self, optimizer, *args, **kwargs):
        """
        :meth:`step` carries out the following two operations:
        ...
        """
        if (not self._enabled):
            return optimizer.first_step(*args, **kwargs)
        ...
        retval = self._maybe_opt_step_first(optimizer, optimizer_state, *args, **kwargs)

        # optimizer_state["stage"] = OptState.STEPPED

        return retval

    def _maybe_opt_step_second(self, optimizer, optimizer_state, *args, **kwargs):
        retval = None
        if not sum(v.item() for v in optimizer_state["found_inf_per_device"].values()):
            retval = optimizer.second_step(*args, **kwargs)
        return retval

    def second_step(self, optimizer, *args, **kwargs):
        """
        :meth:`step` carries out the following two operations:
        ...
        """
        if (not self._enabled):
            return optimizer.second_step(*args, **kwargs)
        ...
        retval = self._maybe_opt_step_second(optimizer, optimizer_state, *args, **kwargs)
