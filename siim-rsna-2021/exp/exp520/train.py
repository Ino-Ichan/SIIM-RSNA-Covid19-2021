...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        images = cv2.resize(images, (600, 600))
...
def train_one_epoch(train_dataloader, model, device, criterion, use_amp, wandb, meters_dict, mode="train"):
    ...
    for step_train, data in enumerate(progress_bar):
        ...
        with autocast(enabled=use_amp):
            output, output_mask6, output_mask7 = model(inputs)
            loss_cls = criterion[0](output, target).mean()
            loss_mask6 = criterion[0](output_mask6, target_mask).mean() + criterion[1](output_mask6, target_mask).mean()
            loss_mask7 = criterion[0](output_mask7, target_mask).mean() + criterion[1](output_mask7, target_mask).mean()
            loss_mask = loss_mask6 * 0.5 + loss_mask7 * 0.5
