...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            bbox_mask_32 = cv2.resize(bbox_mask, (int(self.image_size // 16), int(self.image_size // 16)))[None, :, :]
            bbox_mask_16 = cv2.resize(bbox_mask, (int(self.image_size // 32), int(self.image_size // 32)))[None, :, :]
        else:
        ...
        return {
            ...
            "mask16": torch.tensor(bbox_mask_16, dtype=torch.float),
            "mask32": torch.tensor(bbox_mask_32, dtype=torch.float),
...
def train_one_epoch(train_dataloader, model, device, criterion, use_amp, wandb, meters_dict, mode="train"):
    ...
    for step_train, data in enumerate(progress_bar):
        ...
        target_mask_16 = data["mask16"].to(device)
        target_mask_32 = data["mask32"].to(device)

        bs = inputs.shape[0]

        with autocast(enabled=use_amp):
            output, output_mask5, output_mask6, output_mask7 = model(inputs)
            loss_cls = criterion[0](output, target).mean()
            loss_mask5 = criterion[0](output_mask5, target_mask_32).mean() + criterion[1](output_mask5, target_mask_32).mean()
            loss_mask6 = criterion[0](output_mask6, target_mask_16).mean() + criterion[1](output_mask6, target_mask_16).mean()
            loss_mask7 = criterion[0](output_mask7, target_mask_16).mean() + criterion[1](output_mask7, target_mask_16).mean()
            loss_mask = loss_mask6 * 0.5 + loss_mask7 * 0.25 + loss_mask5 * 0.25
