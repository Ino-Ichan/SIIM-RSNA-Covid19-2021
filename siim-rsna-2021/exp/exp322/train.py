    ...
    def __getitem__(self, index):
        ...
        if bbox_df.iloc[0]["have_box"]:
            for n_box in range(len(bbox_df)):
                ...
                bbox_mask[int(box_info["y"] / original_h):int((box_info["y"]+box_info["h"]) / original_h),
                          int(box_info["x"] / original_w):int((box_info["x"]+box_info["w"]) / original_w)] = 1. # 255.
...
def train_one_epoch(train_dataloader, model, device, criterion, use_amp, wandb, meters_dict, mode="train"):
    ...
    for step_train, data in enumerate(progress_bar):
        ...
        with autocast(enabled=use_amp):
            ...
            loss_cls = loss_cls = 0.8 * criterion[1](output[..., :4], target[..., :4].argmax(-1)).mean()\
                 + 0.2 * criterion[0](output[..., 4:5], target[..., 4:5]).mean()
            loss_mask = criterion[0](output_mask, target_mask).mean()
        ...
        meters_dict["AP"].add(output=output2.detach(), target=target)
...
def get_train_transforms(image_size):
    ...
    albumentations.ShiftScaleRotate(shift_limit=0.1, scale_limit=0.2, rotate_limit=15, border_mode=0, p=0.75),
