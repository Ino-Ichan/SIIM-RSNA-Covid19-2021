...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            bbox_mask = cv2.resize(bbox_mask, (int(self.image_size // 32), int(self.image_size // 32)))[None, :, :]
        else:
...
def train_one_epoch(train_dataloader, model, device, criterion, use_amp, wandb, meters_dict, mode="train"):
    ...
    for step_train, data in enumerate(progress_bar):
        ...
        with autocast(enabled=use_amp):
            ...
            loss_mask = criterion[0](output_mask, target_mask).mean() + criterion[1](output_mask, target_mask).mean()
