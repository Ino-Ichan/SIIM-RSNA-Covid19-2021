...
class Net(nn.Module):
    def __init__(self, name="resnest101e", drop_rate=0):
        super(Net, self).__init__()
        self.model = timm.create_model(name, pretrained=True, num_classes=len(target_columns), drop_rate=drop_rate)
...
def get_train_transforms1(image_size):
    return albumentations.Compose([
...
def get_train_transforms3(image_size):
    return albumentations.Compose([
           albumentations.ShiftScaleRotate(p=0.5),
           albumentations.RandomResizedCrop(image_size, image_size, scale=(0.7, 1), p=1),
           albumentations.HorizontalFlip(p=0.5),
    ...
    for cv in cv_list:
        ...
        for e in range(start_epoch , start_epoch + n_epochs):
            ...
            if e == 7+1:
                ...
                drop_rate = 0.1
                up_image_size = 128
                train_transform = get_train_transforms(img_size+up_image_size)
                val_transform = get_val_transforms(img_size+up_image_size)

                model = Net(name=model_name, drop_rate=drop_rate).to(device)
                # wandb misc
                wandb.watch(model)
                weight = torch.load(model_save_path, map_location=device)
                model.load_state_dict(weight["state_dict"])
                LOGGER.info(f"Eppch {e}, Successfully loaded model, model path: {load_checkpoint}")
                train_dataset = CustomDataset(df=df_train.sample(frac=1.0), image_size=img_size, clahe=clahe, mix=mix,
                                            transform=train_transform, use_npy=use_npy, mode="train")
                train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                                            pin_memory=False, num_workers=n_workers, drop_last=True)

                # plot sample image
                plot_sample_images(train_dataset, sample_img_path, f"train_e{e}", normalize=None)
