...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if bbox_df.iloc[0]["have_box"]:
            for n_box in range(len(bbox_df)):
                ...
                bbox_mask[int(img_h * box_info["y"] / original_h):int(img_h * (box_info["y"]+box_info["h"]) / original_h),
                          int(img_w * box_info["x"] / original_w):int(img_w * (box_info["x"]+box_info["w"]) / original_w)] = 1. # 255.

        # lung crop
        lung_mask = cv2.imread(row.mask_path, cv2.IMREAD_GRAYSCALE)
        lung_mask = (cv2.resize(lung_mask, (images.shape[1], images.shape[0])) > 0.5) * 1
        ...
        if self.transform:
            ...
            aug = self.transform(image=images, mask=mask_mix)
            images = aug['image'].astype(np.float32)                
            mask_mix = aug['mask'].astype(np.float32)

            bbox_mask = mask_mix[:,:,0]
            lung_mask = mask_mix[:,:,1]

        x0, y0, x1, y1 = get_bbox_from_mask(lung_mask)

        images = images[y0:y1, x0:x1]
        bbox_mask = bbox_mask[y0:y1, x0:x1]

        aug = self.transform_resize(image=images, mask=bbox_mask[:,:,None])
        images = aug['image'].transpose(2, 0, 1) / 255                
        bbox_mask = aug['mask']
