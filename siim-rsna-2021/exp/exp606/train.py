...
class Net(nn.Module):
    def __init__(self, name="resnest101e"):
        ...
        swin = timm.create_model(name, True)
        ...
        self.myfc = nn.Linear(swin.head.in_features, len(target_columns))
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            bbox_mask = cv2.resize(bbox_mask, (int(self.image_size // 8), int(self.image_size // 8)))[None, :, :]
        else:
...
def train_one_epoch(train_dataloader, model, device, criterion, use_amp, wandb, meters_dict, mode="train"):
    ...
    for step_train, data in enumerate(progress_bar):
        ...
        with autocast(enabled=use_amp):
            ...
            loss_mask1 = criterion[0](output_mask1, target_mask).mean() + criterion[1](output_mask1, target_mask).mean()
            loss_mask2 = criterion[0](output_mask2, target_mask).mean() + criterion[1](output_mask2, target_mask).mean()
