...
class Net(nn.Module):
    def __init__(self, name="resnest101e"):
        ...
        model = timm.create_model(name, pretrained=True, num_classes=len(target_columns))
    ...
    def forward(self, x, is_emb=False):
        ...
        x = self.avgpool(emb.transpose(1, 2))
        x = self.head(torch.flatten(x, 1)).squeeze(-1)
        if is_emb:
            return x, emb
        else:
            return x
...
def train_one_epoch(train_dataloader, model, premodel, device, criterion, use_amp, wandb, meters_dict, mode="train"):
...
def val_one_epoch(val_dataloader, model, device, wandb, meters_dict, mode="val"):
    ...
    for step_val, data in enumerate(progress_bar):
        ...
        with torch.no_grad():
            output = model(inputs)
            loss = criterion[0](output, target).mean()
    ...
    for cv in cv_list:
        ...
        for e in range(start_epoch , start_epoch + n_epochs):
            if e > 0:
                ...
                train_one_epoch(train_dataloader, model, premodel, device, criterion, use_amp, wandb, meters_dict)
