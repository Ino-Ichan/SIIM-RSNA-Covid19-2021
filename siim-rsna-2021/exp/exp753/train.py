...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if row.external == "original":
            ...
            if bbox_df.iloc[0]["have_box"]:
                for n_box in range(len(bbox_df)):
                    ...
                    bbox_mask[int(img_h * box_info["y"] / original_h):int(img_h * (box_info["y"]+box_info["h"]) / original_h),
                            int(img_w * box_info["x"] / original_w):int(img_w * (box_info["x"]+box_info["w"]) / original_w)] = 255.
        else:
            bbox_mask = np.load(row.mask_path) * 255
            bbox_mask = bbox_mask.astype(np.uint8)
            bbox_mask = cv2.resize(bbox_mask, (512, 512))

        if self.transform is not None:
            aug = self.transform(image=images, mask=bbox_mask)                
            images_only = aug['image'].astype(np.float32).transpose(2, 0, 1) / 255                
            bbox_mask = aug['mask'].astype(np.float32) / 255
            # bbox_mask = cv2.resize(bbox_mask, (int(self.image_size // 16), int(self.image_size // 16)))[None, :, :]
            bbox_mask1 = cv2.resize(bbox_mask, (int(self.image_size // 16), int(self.image_size // 16)))[None, :, :]
            bbox_mask2 = cv2.resize(bbox_mask, (int(self.image_size // 32), int(self.image_size // 32)))[None, :, :]
        else:
        ...
        return {
            ...
            "mask1": torch.tensor(bbox_mask1, dtype=torch.float),
            "mask2": torch.tensor(bbox_mask2, dtype=torch.float),
...
def train_one_epoch(train_dataloader, model, device, criterion, use_amp, wandb, meters_dict, mode="train"):
    ...
    for step_train, data in enumerate(progress_bar):
        ...
        target_mask1 = data["mask1"].to(device)
        target_mask2 = data["mask2"].to(device)

        bs = inputs.shape[0]

        with autocast(enabled=use_amp):
            output, output_mask1, output_mask2, output_mask3 = model(inputs)
            loss_cls = criterion[0](output, target).mean()
            loss_mask1 = criterion[0](output_mask1, target_mask1).mean() + criterion[1](output_mask1, target_mask1).mean()
            loss_mask2 = criterion[0](output_mask2, target_mask2).mean() + criterion[1](output_mask2, target_mask2).mean()
            loss_mask3 = criterion[0](output_mask3, target_mask2).mean() + criterion[1](output_mask3, target_mask2).mean()
            loss_mask = 0.25 * loss_mask1 + 0.5 * loss_mask2 + 0.25 * loss_mask3
        ...
        meters_dict["AP"].add(output=output.detach(), target=(target > 0.5)*1)
...
if __name__ == "__main__":
    ...
    for cv in cv_list:
        ...
        df_val = df[(df.cv == cv) & (df.external == 'original')].reset_index(drop=True)
