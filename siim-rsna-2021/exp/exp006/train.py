...
class CustomDataset(Dataset):
    def __init__(self,
        ...
        if self.clahe or self.mix:
            ...
            self.clahe_transform = cv2.createCLAHE(clipLimit=5, tileGridSize=(8, 8))
    ...
    def __getitem__(self, index):
        ...
        # images = cv2.cvtColor(images.astype(np.float32), cv2.COLOR_GRAY2RGB)


        if self.clahe:
            single_channel = (255 * images[:, :, 0]).astype(np.uint8)
        ...
        if self.transform is not None:
            ...
            # else:
            # # images = self.transform(image=images.astype(np.float32))['image']
            #     images = self.transform(image=images)['image']
        else:
