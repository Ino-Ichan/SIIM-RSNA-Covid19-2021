...
class Net(nn.Module):
    def __init__(self, name="resnest101e"):
        ...
        aux_params=dict(
            ...
            classes=len(target_columns),                 # define number of output labels
        )
        self.model = smp.Unet(
    ...
    def forward(self, x):
        ...
        return label.squeeze(-1), mask.squeeze(1)
...
def train_one_epoch(train_dataloader, model, device, criterion, use_amp, wandb, meters_dict, mode="train"):
    ...
    for step_train, data in enumerate(progress_bar):
        ...
        with autocast(enabled=use_amp):
            output, out_mask = model(inputs)
            cls_loss = criterion[0](output, target).mean()
            seg_loss = criterion[1](out_mask, target_mask).mean()
        ...
        meters_dict["cls_loss"].add(cls_loss.item(), n=bs)
        meters_dict["seg_loss"].add(seg_loss.item(), n=bs)
    ...
    LOGGER.info(f"Train cls_loss: {meters_dict['cls_loss'].value()[0]}")
    LOGGER.info(f"Train seg_loss: {meters_dict['seg_loss'].value()[0]}")
    ...
    wandb.log({
        ...
        "Cls_Loss/train": meters_dict['cls_loss'].value()[0],
        "Seg_Loss/train": meters_dict['seg_loss'].value()[0],
...
def val_one_epoch(val_dataloader, model, device, wandb, meters_dict, mode="val"):
    ...
    LOGGER.info(f"Val cls_loss: {meters_dict['cls_loss'].value()[0]}")
    LOGGER.info(f"Val seg_loss: {meters_dict['seg_loss'].value()[0]}")
    ...
    log_dict = {
        ...
        "Cls_Loss/val": meters_dict['cls_loss'].value()[0],
        "Seg_Loss/val": meters_dict['seg_loss'].value()[0],
