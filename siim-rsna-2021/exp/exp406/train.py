...
#         self.myfc = nn.Linear(self.enet.head.in_features, len(target_columns))
...
def train_one_epoch(train_dataloader, model, device, criterion, use_amp, wandb, meters_dict, mode="train"):
    ...
    for step_train, data in enumerate(progress_bar):
        ...
        with autocast(enabled=use_amp):
            output, mask = model(inputs)
            loss_label = criterion(output, target).mean()
            loss_mask = criterion(mask, target_mask).mean()
            loss = loss_label * WEIGHT_LABEL + loss_mask * WEIGHT_SEG
    ...
    wandb.log({
        ...
        f"Loss_Mask/train_cv{cv}": meters_dict['loss_mask'].value()[0],
        f"Loss_Total/train_cv{cv}": meters_dict['loss_total'].value()[0],
...
if __name__ == "__main__":
    ...
    for cv in cv_list:
        ...
        for e in range(start_epoch , start_epoch + n_epochs):
            ...
            score, old_score = val_one_epoch(val_dataloader, model, device, wandb, meters_dict)
            scheduler.step()
