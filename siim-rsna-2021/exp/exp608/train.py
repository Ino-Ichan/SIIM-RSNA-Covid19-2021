...
class Net(nn.Module):
    def __init__(self, name="resnest101e"):
        ...
        model = timm.create_model(name, True)
        ...
        self.b1 = model.layer1
        self.b2 = model.layer2
        self.b3 = model.layer3
        self.b4 = model.layer4

        self.global_pool = model.global_pool
        self.dropout = nn.Dropout(0.5)
        self.myfc = nn.Linear(model.fc.in_features, len(target_columns))
...
def train_one_epoch(train_dataloader, model, device, criterion, use_amp, wandb, meters_dict, mode="train"):
    ...
    for step_train, data in enumerate(progress_bar):
        ...
        with autocast(enabled=use_amp):
            output, output_mask1, output_mask2 = model(inputs)
            ...
            loss_mask = (loss_mask1 + loss_mask2) / 2
...
def val_one_epoch(val_dataloader, model, device, wandb, meters_dict, mode="val"):
    ...
    for step_val, data in enumerate(progress_bar):
        ...
        inputs = data["image"].to(device)
        target = data["target"].to(device)
        # target_mask1 = F.interpolate(data["mask1"].to(device), size=512//16, mode='bilinear', align_corners=False)
        # target_mask2 = F.interpolate(data["mask2"].to(device), size=512//32, mode='bilinear', align_corners=False)
        target_mask1 = data["mask1"].to(device)
        target_mask2 = data["mask2"].to(device)
