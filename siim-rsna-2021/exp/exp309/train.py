...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_cat = np.argmax(label).astype(np.float16)
        return {
            "image": torch.tensor(images, dtype=torch.float),
            # "image": images,
            "target": torch.tensor(label, dtype=torch.float),
            "target_cat": torch.tensor(label_cat, dtype=torch.long)
        }
...
def train_one_epoch(train_dataloader, model, device, criterion, use_amp, wandb, meters_dict, mode="train"):
    ...
    for step_train, data in enumerate(progress_bar):
        ...
        target_cat = data["target_cat"].to(device)

        bs = inputs.shape[0]

        with autocast(enabled=use_amp):
            output = model(inputs)
            loss = criterion(output, target_cat).mean()
