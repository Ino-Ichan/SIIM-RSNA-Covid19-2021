...
if __name__ == "__main__":
    ...
    for cv in cv_list:
        ...
        wandb.init(config=cfg, tags=[cfg['exp_name'], f"cv{cv}", model_name],
                   project='siim-rsna-covid19-2021-3', entity='inoichan',
                   name=f"{cfg['exp_name']}_cv{cv}_{model_name}", reinit=True)

        df_train = df[df[fold_name] != cv].reset_index(drop=True)
        df_val = df[df[fold_name] == cv].reset_index(drop=True)
        val_index = df[df[fold_name] == cv].index
